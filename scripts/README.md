...
Raven also drops a daily `df` + CPU-load snapshot at
<https://gannet.fish.washington.edu/v1_web/owlshell/bu-github/ghr.log>,
generated by a cron on raven around 7am and synced to gannet's public web
root. Because that URL is on gannet (public), `check_servers.py` fetches and
parses it (`fetch_raven_stats`) under **either** profile, unlike the live
TCP/8787 check, which only the internal profile can reach.

The parsed result is written as a top-level `raven_stats` field (not nested
under `hosts`), separate from the up/down lights:
...
It has to stay out of `hosts` because `hosts` entries are merged by picking
the freshest whole *document*, and the external prober can run more often
